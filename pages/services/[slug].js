...
{!booking && <div className={styles.serviceSticky}><div><small>{service.title}</small><strong>{price}</strong></div>{service.externalLink ? <a href={service.externalLink} target={service.externalLink.startsWith("/") ? undefined : "_blank"} rel="noopener noreferrer">{service.ctaLabel || "Book service"} <FiArrowUpRight /></a> : <button onClick={() => setBooking(true)}>Book service <FiArrowUpRight /></button>}</div>}
